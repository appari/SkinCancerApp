...
class Page3ViewController: UIViewController {
    ...
    @objc func saveImageButtonPressed(_ sender: UIButton) {
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self] _ in
            if let folderName = alert.textFields?[0].text, !folderName.isEmpty,
               let imageName = alert.textFields?[1].text, !imageName.isEmpty {
    ...
    func saveImageToDocuments(image: UIImage, folderName: String, imageName: String) {
        ...
        do {
            ...
            let imageURL = folderURL.appendingPathComponent("\(imageName).png")
            if let data = image.pngData() {
                try data.write(to: imageURL)
                print("Image saved at path: \(imageURL.path)")
                
                // Notify with image path
